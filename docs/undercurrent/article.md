...
The most useful thing a review did, though, was subtract. A finding came in saying the avatars' bob phase was seeded by a running counter over a map keyed by draw order — true, and it meant one person changing spot re-dealt everyone else's phase, popping bystanders about nine pixels while the rest of the scene glided. It also claimed this desynced two windows permanently. The verifier checked and it doesn't: members are sorted by creation date before they reach the canvas, so both windows pop identically at the same instant. Same fix either way — the phase now hashes from the person's id, so it belongs to them and no one else can re-deal it — but without that correction I'd have spent the afternoon hunting a synchronisation bug that was never there. A report you can't challenge is worth less than one you can.
